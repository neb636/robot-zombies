...
# PixelLab Asset Pipeline — Quiet Machines
...
```
ToolSearch(query: "select:mcp__pixellab__create_character,mcp__pixellab__animate_character,mcp__pixellab__get_character,mcp__pixellab__create_topdown_tileset,mcp__pixellab__get_topdown_tileset,mcp__pixellab__create_tiles_pro,mcp__pixellab__get_tiles_pro,mcp__pixellab__create_map_object,mcp__pixellab__get_map_object,mcp__pixellab__list_characters,mcp__pixellab__list_topdown_tilesets,mcp__pixellab__list_tiles_pro", max_results: 12)
```

Only load the subset you need for the current asset type — don't load everything every time.

### 3. PixelLab @docs references (optional but improves output quality)

PixelLab MCP supports injecting documentation context into generation prompts via MCP resource URIs. Include them in the `description` field of your tool calls:

| Asset type | URI to append |
|------------|---------------|
| Top-down tilesets | `@pixellab://docs/python/wang-tilesets` |
| Sidescroller tilesets | `@pixellab://docs/godot/sidescroller-tilesets` |
| Isometric tiles | `@pixellab://docs/godot/isometric-tiles` |

Example: append the URI on a new line at the end of `lower_description`. These are MCP resource URIs, not web URLs — do not fetch them with WebFetch.

### 4. Art-style constants (apply to every generation)

**Camera perspective:** Quiet Machines uses a **3/4 view RPG perspective** (like Chrono Trigger / FF6) — not isometric. Characters and environments are drawn with a slight top-down angle showing both the top and front face of objects, with no strict 45° diamond grid. This perspective must be consistent across all generated assets.
...
On wake, call the matching `get_*` tool for each submitted job ID.

**Image retrieval rules — read carefully:**

- `get_character` / `get_topdown_tileset` / `get_map_object` return the image data directly in the tool response. Write the returned image data to file using the `Write` tool.
- `get_tiles_pro` and `get_sidescroller_tileset` return a **ZIP download URL** containing multiple PNGs. Use `curl` to download the ZIP, then `unzip` to extract — never save a ZIP with a `.png` extension.
  ```bash
  curl -s "<download_url>" -o staging_dir/asset.zip
  unzip staging_dir/asset.zip -d staging_dir/
  rm staging_dir/asset.zip
  ```
- **Never use the `Read` tool to display generated images.** The Anthropic vision API will return `400 Could not process image` if the file is a ZIP or large PNG. Just report the file paths.
...
- **The PNG files** (spritesheet, individual directions, tileset tiles — whatever PixelLab returned after extraction).
...
Adapt these; don't copy blindly. Character personalities and visual cues come from the `## Characters` table in `AGENTS.md`.
