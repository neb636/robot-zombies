...
export class SubwayScene extends Phaser.Scene {
  ...
  private _buildNPCs(): void {
    ...
    // Maya sprite
    const mayaTexture = this.textures.exists('maya_idle') ? 'maya_idle' : '__DEFAULT';
    this._mayaSprite = this.add.sprite(MAYA_X, MAYA_Y, mayaTexture)
      .setScale(0.70).setDepth(5);
    if (this.anims.exists('maya-idle')) {
      this._mayaSprite.play('maya-idle');
    }

    // Label above Maya
    this.add.text(MAYA_X, MAYA_Y - 52, 'MAYA', {
