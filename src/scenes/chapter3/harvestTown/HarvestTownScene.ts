...
import D from './dialogue.json';
import { CORA_NPC }  from '../../../characters/npcs/cora/Npc.js';
import { GIDEON_NPC } from '../../../characters/npcs/gideon/Npc.js';
...
export class HarvestTownScene extends Phaser.Scene {
  ...
  private _buildNPCs(): void {
    ...
    this._coraSprite = this.add.rectangle(CORA_NPC.x, CORA_NPC.y, CORA_NPC.width, CORA_NPC.height, CORA_NPC.color).setDepth(5);
    this.add.text(CORA_NPC.x, CORA_NPC.y - 22, 'CORA', {
      fontFamily: 'monospace', fontSize: '8px', color: '#8899aa',
    }).setOrigin(0.5).setDepth(6);

    // Gideon — warm tan
    this._gideonSprite = this.add.rectangle(GIDEON_NPC.x, GIDEON_NPC.y, GIDEON_NPC.width, GIDEON_NPC.height, GIDEON_NPC.color).setDepth(5);
    this.add.text(GIDEON_NPC.x, GIDEON_NPC.y - 24, 'GIDEON', {
  ...
  private _checkNPCProximity(): void {
    ...
    const distCora   = Math.hypot(px - CORA_NPC.x,  py - CORA_NPC.y);
    const distGideon = Math.hypot(px - GIDEON_NPC.x, py - GIDEON_NPC.y);
