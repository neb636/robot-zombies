...
import {
  ...
  drawPrologueRoomV2,
} from './PrologueV2RoomRenderer.js';
...
/**
 * PrologueV2Scene — "Before the Fall" (June 2028, Boston MA)
 * Sprite-based visual rework of PrologueScene.
 */
export class PrologueV2Scene extends Phaser.Scene {
  ...
  // TV coordinates (match renderer placement)
  private readonly _tvX = 520;
  private readonly _tvY = 40;

  private _tvScreenFlash(): void {
    const flash = this.add.rectangle(this._tvX, this._tvY + 8, 104, 40, 0x003399)
  ...
  private _addTVOverlay(): void {
    this._tvLabel = this.add.text(this._tvX, this._tvY, 'SUPERINTELLIGENCE INC — LIVE', {
      fontFamily: 'monospace', fontSize: '5px', color: '#3366cc',
    }).setOrigin(0.5).setDepth(9);

    this._tvBroadcastBar = this.add.rectangle(this._tvX, this._tvY + 30, 104, 6, 0x003366)
