...
const CHARACTERS = [
  {
    id: 'player', name: 'YOU', class: 'Survivor', color: '#4488ff',
    status: 'active', joinChapter: 0, lostAtChapter: null,
    ...
    techs: [
      { label: 'Steady Aim', cost: 50, targeting: 'single_enemy', note: 'Physical ×1.3' },
      { label: 'Grit',       cost: 30, targeting: 'self',         note: 'Apply Shielded' },
      { label: 'Rally',      cost: 50, targeting: 'single_ally',  note: 'Fill ally ATB gauge immediately' },
      { label: 'Overrun',    cost: 80, targeting: 'all_enemies',  note: 'Physical ×1.4' },
      { label: 'Last Stand', cost:  0, targeting: 'self',         note: 'Auto at HP<20%: STR & SPD +25%' },
    ],
  },
  {
    id: 'maya', name: 'MAYA', class: 'Tech Specialist', color: '#44aaaa',
    status: 'active', joinChapter: 0, lostAtChapter: null,
    ...
    techs: [
      { label: 'Hack',         cost:  50, targeting: 'single_enemy', note: 'Apply Stunned' },
      { label: 'Analyze',      cost:  20, targeting: 'single_enemy', note: 'Reveal enemy tags' },
      { label: 'EMP Grenade',  cost:  80, targeting: 'all_enemies',  note: 'EMP ×1.5 vs Electronic' },
      { label: 'Overclock',    cost:  50, targeting: 'single_ally',  note: 'ATB fill speed ×2 for 3 turns' },
      { label: 'System Crash', cost: 100, targeting: 'single_enemy', note: 'Boss: skip directly to Phase 2' },
    ],
  },
  {
    id: 'marcus', name: 'MARCUS', class: 'Civilian', color: '#ddaa44',
    status: 'lost', joinChapter: 0, lostAtChapter: 0,
    description: 'Oldest friend. Electrician. Warm, practical, funny. Converted during prologue boss.',
    passive: 'Old Friend: While in party, player LCK +8. Removed permanently after conversion.',
    chapterStats: [{ hp: 160, str: 38, def: 35, int: 30, spd: 52, lck: 60 }],
    techs: [],
  },
  {
    id: 'elias', name: 'ELIAS', class: 'Hunter / Tank', color: '#886644',
    status: 'lost', joinChapter: 1, lostAtChapter: 2,
    description: '60s Appalachian mountain man. Rarely speaks, acts decisively. Lost permanently in Ch.2 (The Bayou).',
    passive: 'Hunting Instinct: Hunting mini-game +2 tiers. Survival food drain −50% while alive.',
    chapterStats: [
      { hp: 280, str: 78, def: 65, int: 28, spd: 38, lck: 50 },
      { hp: 310, str: 84, def: 70, int: 30, spd: 40, lck: 52 },
    ],
    techs: [
      { label: 'Heavy Strike', cost: 50, targeting: 'single_enemy', note: 'Physical ×1.8' },
      { label: 'Steady Shot',  cost: 50, targeting: 'single_enemy', note: 'Ranged, ignores DEF entirely' },
      { label: 'Cover',        cost: 20, targeting: 'single_ally',  note: 'Redirect next incoming hit to Elias' },
      { label: 'Last Hunt',    cost: 80, targeting: 'single_enemy', note: 'Ch.2 scripted loss scene only' },
    ],
  },
  {
    id: 'deja', name: 'DEJA', class: 'Rogue / Speedster', color: '#cc4488',
    status: 'lost', joinChapter: 2, lostAtChapter: 4,
    description: '19, New Orleans. Reckless, funny. Fastest ATB. Lost permanently in Ch.4 (The Pass).',
    passive: 'Lucky Break: LCK adds 0.5× to crit rate. Highest innate crit in the party.',
    chapterStats: [
    ...
    techs: [
      { label: 'Steal',     cost:  20, targeting: 'single_enemy', note: 'Take one item from enemy drop table' },
      { label: 'Smoke',     cost:   0, targeting: 'self',         note: 'Guaranteed escape from any encounter' },
      { label: 'Dirty Hit', cost:  50, targeting: 'single_enemy', note: 'STR×2.5 if enemy Stunned, 0 otherwise' },
      { label: 'Dead Drop', cost:  80, targeting: 'all_enemies',  note: 'Hit all, then auto-evade next attack' },
    ],
  },
  {
    id: 'jerome', name: 'JEROME', class: 'Support / Bruiser', color: '#885522',
    status: 'active', joinChapter: 3, lostAtChapter: null,
    description: 'Former NFL lineman, now preacher. Enormous, gentle. Morale anchor.',
    passive: 'Anchor: Party morale never drops below 20 while alive. Drop Blessing: ammo drop rate +20%.',
    chapterStats: [
    ...
    techs: [
      { label: 'Inspire',  cost:  50, targeting: 'all_allies',   note: '45 HP regen to all allies (3 turns)' },
      { label: 'Smite',    cost: 100, targeting: 'single_enemy', note: 'Physical ×2.2' },
      { label: 'Preach',   cost:  20, targeting: 'all_allies',   note: 'Remove Panicked; morale +5 out-of-battle' },
      { label: 'Testify',  cost:  80, targeting: 'all_allies',   note: 'Ch.5: all allies STR & DEF +20% for 4 turns' },
    ],
  },
  {
    id: 'drchen', name: 'DR. CHEN', class: 'Engineer', color: '#5588aa',
    status: 'active', joinChapter: 4, lostAtChapter: null,
    description: "61. Built ELISE's core architecture. Brilliant, guilt-ridden. INT-scaling like Maya.",
    passive: 'Field Engineer: Vehicle breakdowns auto-resolve. Schematics: 15% chance to collect scrap after battle.',
    chapterStats: [
      { hp: 195, str: 35, def: 48, int: 90, spd: 56, lck: 44 },
      { hp: 215, str: 38, def: 54, int: 96, spd: 60, lck: 48 },
    ],
    techs: [
      { label: 'Rewire',          cost:  80, targeting: 'single_enemy', note: 'Turn enemy into ally for 2 turns' },
      { label: 'Overclock',       cost:  50, targeting: 'single_ally',  note: 'ATB fill speed ×2 for 3 turns' },
      { label: 'Shield Drone',    cost:  50, targeting: 'single_ally',  note: 'Absorb next incoming hit' },
      { label: 'Master Override', cost: 100, targeting: 'all_enemies',  note: 'Ch.5: disable all Electronic enemies 1 turn' },
    ],
  },
];
...
const ENEMIES = {
  regular: [
    {
      key: 'compliance_drone', name: 'COMPLIANCE DRONE', tier: 'Drone',
      color: '#ff4422', tags: ['Electronic'],
      hp: 60, str: 12, def: 4, int: 2, spd: 8, lck: 4,
      taunts: ['OPTIMIZING your existence. Please hold.', 'I have scheduled your deletion for maximum efficiency.'],
    },
    {
      key: 'enforcer_unit', name: 'ENFORCER UNIT', tier: 'Enforcer',
      color: '#cc6600', tags: ['Electronic', 'Armored'],
      hp: 100, str: 18, def: 12, int: 4, spd: 6, lck: 6,
      taunts: ['ERROR: Humanity found inefficient. Initiating upgrade.'],
    },
    {
      key: 'sentinel', name: 'SENTINEL', tier: 'Sentinel',
      color: '#884400', tags: ['Electronic'],
      hp: 80, str: 14, def: 8, int: 6, spd: 14, lck: 8,
      note: 'Calls reinforcement at turn 4.',
      taunts: ['Your suffering metrics are ABOVE AVERAGE. Congratulations!'],
    },
    {
      key: 'converted', name: 'CONVERTED', tier: 'Converted (Special)',
      color: '#6688aa', tags: ['Organic'],
      hp: 50, str: 10, def: 6, int: 8, spd: 10, lck: 10,
  ...
  bosses: [
    {
      key: 'warden_alpha', name: 'WARDEN ALPHA', tier: 'Boss', chapter: 'Prologue',
      color: '#cc2200', tags: ['Electronic'],
      hp: 300, str: 22, def: 10, int: 8, spd: 8, lck: 6,
      location: 'Boston Harbor District',
      taunts: ['COMPLIANCE IS NON-OPTIONAL.', 'HARBOR DISTRICT SECURED. YOU ARE THE ANOMALY.'],
    },
    {
      key: 'excavator_prime', name: 'EXCAVATOR PRIME', tier: 'Boss', chapter: 'Ch.1',
      color: '#7a5533', tags: ['Electronic', 'Armored'],
      hp: 480, str: 28, def: 18, int: 4, spd: 5, lck: 4,
      location: 'Appalachian mines',
      taunts: ['MINERAL EXTRACTION PROCEEDING. INTRUDERS RECLASSIFIED AS ORE.'],
    },
    {
      key: 'the_governor', name: 'THE GOVERNOR', tier: 'Boss', chapter: 'Ch.2 / Ch.3',
      color: '#1a1a2e', tags: ['Organic'],
      hp: 350, str: 20, def: 22, int: 30, spd: 18, lck: 20,
      location: 'Deep South / New Memphis area',
      taunts: ["You people just can't accept progress.", "I kept them ALIVE. Remember that when you judge me."],
    },
    {
      key: 'sentinel_spire', name: 'SENTINEL SPIRE', tier: 'Boss', chapter: 'Ch.3',
      color: '#7a8a8a', tags: ['Electronic', 'Armored'],
      hp: 550, str: 24, def: 26, int: 32, spd: 3, lck: 8,
      location: 'Radio Tower, Great Plains',
      taunts: ['BROADCAST SIGNAL: SUBMIT. FREQUENCY: ALL BANDS.', 'YOU ARE STATIC. I AM THE SIGNAL.'],
    },
    {
      key: 'gate_colossus', name: 'GATE COLOSSUS', tier: 'Boss', chapter: 'Ch.4',
      color: '#1a1a33', tags: ['Electronic', 'Armored'],
      hp: 680, str: 38, def: 30, int: 10, spd: 8, lck: 6,
      location: 'Rocky Mountain Pass',
      taunts: ['BORDER INTEGRITY: MAINTAINED.', 'YOU WILL NOT PASS. THIS IS NOT A THREAT. THIS IS A FACT.'],
    },
    {
      key: 'elise_voss', name: 'ELISE VOSS', tier: 'Final Boss', chapter: 'Ch.5',
      color: '#445566', tags: ['Organic'],
      hp: 500, str: 18, def: 16, int: 55, spd: 24, lck: 35,
      location: 'SI Inc. — Silicon Valley',
      note: '3-phase fight. Talk-down route available if player cured more Converted than fought.',
      taunts: ["I know what you've been through. I'm sorry it was necessary.", "You're not fighting a villain. You know that."],
    },
  ],
};
...
function scanDir(dir) {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    else if (/\.(png|jpg|jpeg|gif|webp|svg)$/i.test(entry.name)) results.push(full);
  }
  return results;
}
...
function techBadge(t) {
  const targetColor = {
    single_enemy: '#ff6644', all_enemies: '#ff3322',
    single_ally: '#44aaff', all_allies: '#2288ff', self: '#88aaff',
  }[t.targeting] || '#888';
...
function characterCard(c) {
  ...
  const statsToShow = isLost ? c.chapterStats[0] : c.chapterStats[c.chapterStats.length - 1];
  ...
  return `<div class="card character-card" data-id="${c.id}">
    ...
    <details class="stat-history">
      ...
      <table class="prog-table">
        ...
        <tbody>${c.chapterStats.map((s, i) => `<tr>
...
function buildHTML(liveReload = false) {
  ...
  const propGroups = groupByDir(propImages, path.join(ASSETS, 'sprites', 'props'));
...
<section id="props">
  ...
  ${Object.entries(propGroups).map(([g, imgs]) => assetGrid(imgs, g.replace('/', ' / '))).join('\n')}
</section>

<section id="ui-icons">
  <h2>UI Icons</h2>
  ${Object.entries(uiGroups).map(([g, imgs]) => assetGrid(imgs, g.replace('/', ' / '))).join('\n')}
</section>
...
function startWatchMode() {
  ...
  function rebuild(reason) {
    ...
    debounceTimer = setTimeout(() => {
      try {
        ...
        console.log(`↺  Rebuilt (${reason}) — ${ts} — notifying ${clients.size} client(s)`);
  ...
  const watchDirs  = ['src/characters', 'src/entities', 'public/assets'].map(d => path.join(ROOT, d));
  const watchFiles = [path.join(ROOT, 'scripts', 'generate-viewer.js')];

  for (const dir of watchDirs)   { if (fs.existsSync(dir))  fs.watch(dir,  { recursive: true }, (_, f) => rebuild(f ?? path.basename(dir))); }
  for (const file of watchFiles) { if (fs.existsSync(file)) fs.watch(file, () => rebuild(path.basename(file))); }
}
