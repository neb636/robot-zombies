...
- **Level Builder** (`tools/level-builder/`) — local AI-assisted tool for designing room layouts; see section below
...
A standalone local tool for AI-assisted interior room layout design. Not part of the game bundle — run separately.

**Start:**
```bash
npm run level-builder   # http://localhost:3456
```

**Workflow:**
1. Type a room description (e.g. *"corporate office with cubicles, a cafeteria, and hallways"*)
2. Click **Generate** — server filters `metadata.json` to relevant asset groups and calls the Claude CLI (`claude --print --model sonnet`), which returns 3 distinct layout variations as JSON
3. Browse variations via tabs; click objects to reposition, relabel, or delete them
4. **Export Active Layout** downloads a `LevelLayout` JSON ready to load in a Phaser scene

**Output (`LevelLayout` v1):**
```json
{
  "version": 1,
  "name": "...",
  ...
  "height": 600,
  "layers": {
    "floor":   [ { "assetPath": "extracted-assets/...", "x": 0, "y": 0, "w": 48, "h": 48 } ],
    "object":  [ ... ],
    "overlay": [ ... ]
  }
}
```
`object` and `overlay` layers are depth-sorted by `(y + h)` for isometric rendering.

**Key files:**
| File | Role |
|------|------|
| `tools/level-builder/server.ts` | Express server; loads `metadata.json`, filters catalog, calls Claude CLI, validates response |
| `tools/level-builder/editor.js` | Browser UI — canvas interactions, palette, variation tabs |
| `tools/level-builder/renderer.js` | Canvas rendering (floor → objects → overlay, depth sort) |
| `tools/level-builder/types.ts` | Shared types (`LevelLayout`, `PlacedObject`, `Catalog`, etc.) |
| `metadata.json` | Asset metadata index (generated by `npm run analyze`) |

**Dependency:** requires the `claude` CLI to be installed and authenticated (`claude --version` should work). The server spawns it as a subprocess.
